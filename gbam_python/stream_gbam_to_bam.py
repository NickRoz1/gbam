...
from preprocess import decompress_readnames_from_tokenizer
...
uncompressed_meta_json = brotli.decompress(mm.read())
meta_json = json.loads(uncompressed_meta_json.decode('utf-8'))
...
class OptimizedVarColumn:
    ...
    def get_cumulative(self, block_idx):
        ...
        for j in range(start, start + num_items):
            if j >= self.total_items:
                raise IndexError(f"Index {j} out of range for {self.field}, total: {self.total_items}")
    ...
    def get_item(self, i):
        ...
        print(f"rec_in_block = {rec_in_block}, len(cum_list) = {len(cum_list)}, i = {i}, block_idx = {block_idx}")
...
compressed_readnames = columns["ReadName"]
vocab = meta_json["readname_vocab"]
# compressed_readname_blocks = b''.join(
#     columns["ReadName"].get_decompressed_block(i)
#     for i in range(len(columns["ReadName"].blocks))
# )
# read_names = decompress_readnames_from_tokenizer(compressed_readname_blocks, meta_json["readname_vocab"])
...
with pysam.BGZFile(bam_input_path, "wb") as bam_file:
    ...
    for rec_i in range(records_num):
        ...
        # read_name = read_names[rec_i].encode("utf-8")
