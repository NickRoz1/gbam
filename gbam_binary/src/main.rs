...
fn convert(args: Cli, full_command: String) {
    ...
    let out_path = args
        ...
        .unwrap();
    if args.sort {
        bam_sort_to_gbam(in_path, out_path, Codecs::Gzip, args.sort_temp_mode, args.temp_dir, full_command, args.index_sort);
    } else {
        bam_to_gbam(in_path, out_path, Codecs::Gzip, full_command);
    }
}
