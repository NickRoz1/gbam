...
int write_bam_record(Writer *writer, bam1_t *aln) {
    ...
    // Variable sized fields
    {
        ...
        columns[COLUMNTYPE_read_name].data[columns[COLUMNTYPE_read_name].cur_ptr] += aln->core.l_qname;
        WRITE_INDEX_COLUMN(COLUMNTYPE_read_name)
        
        memcpy(&columns[COLUMNTYPE_cigar].data[columns[COLUMNTYPE_cigar].cur_ptr], bam_get_cigar(aln), aln->core.n_cigar<<2);
        columns[COLUMNTYPE_cigar].data[columns[COLUMNTYPE_cigar].cur_ptr] += aln->core.n_cigar<<2;
        ...
        columns[COLUMNTYPE_seq].data[columns[COLUMNTYPE_seq].cur_ptr] += (((aln)->core.l_qseq + 1)>>1);
        ...
        columns[COLUMNTYPE_qual].data[columns[COLUMNTYPE_qual].cur_ptr] += aln->core.l_qseq;
        ...
        columns[COLUMNTYPE_tags].data[columns[COLUMNTYPE_tags].cur_ptr] += bam_get_l_aux(aln);
